...
function addToTotalModelUsage(
  ...
  modelUsage.inputTokens += normalizedUsage.input_tokens
  modelUsage.outputTokens += normalizedUsage.output_tokens
  modelUsage.cacheReadInputTokens += normalizedUsage.cache_read_input_tokens ?? 0
  modelUsage.cacheCreationInputTokens +=
    normalizedUsage.cache_creation_input_tokens ?? 0
  modelUsage.webSearchRequests +=
    normalizedUsage.server_tool_use?.web_search_requests ?? 0
...
export function addToTotalSessionCost(
  ...
  const normalizedUsage = normalizeUsage(usage)
  const modelUsage = addToTotalModelUsage(cost, normalizedUsage, model)
  ...
  getTokenCounter()?.add(normalizedUsage.input_tokens, {
    ...attrs,
    type: 'input',
  })
  getTokenCounter()?.add(normalizedUsage.output_tokens, {
    ...attrs,
    type: 'output',
  })
