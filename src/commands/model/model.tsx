...
import { fetchLmStudioModels, switchLmStudioModel, type SwitchLmStudioModelResult } from '../../services/lmStudio/modelManagement.js';
async function prepareLmStudioSwitch(model: string | null): Promise<SwitchLmStudioModelResult | null> {
  if (!model || !isLmStudioBackend()) {
    return null;
  }
  return await switchLmStudioModel(model);
}
function appendLmStudioSwitchDetails(message: string, switchResult: SwitchLmStudioModelResult | null): string {
  if (!switchResult) {
    return message;
  }
  if (switchResult.unloadedInstanceIds.length > 0) {
    message += ` \xB7 Unloaded ${switchResult.unloadedInstanceIds.length} ${switchResult.unloadedInstanceIds.length === 1 ? 'other model' : 'other models'}`;
  }
  if (!switchResult.alreadyLoaded) {
    message += ' \xB7 Loaded in LM Studio';
  }
  return message;
}
function ModelPickerWrapper(t0) {
  ...
  if ($[3] !== isFastMode || $[4] !== mainLoopModel || $[5] !== onDone || $[6] !== setAppState) {
    t2 = async function handleSelect(model, effort) {
      ...
      let lmStudioSwitchResult = null;
      try {
        lmStudioSwitchResult = await prepareLmStudioSwitch(model);
      } catch (error) {
        onDone(`Failed to switch LM Studio model: ${(error as Error).message}`, {
          display: 'system'
        });
        return;
      }
      ...
      message = appendLmStudioSwitchDetails(message, lmStudioSwitchResult);
      onDone(message);
    };
    $[3] = isFastMode;
    $[4] = mainLoopModel;
...
function SetModelAndClose({
  ...
  React.useEffect(() => {
    async function handleModelChange(): Promise<void> {
      ...
      if (isLmStudioBackend()) {
        try {
          ...
          const lmStudioModel = lmStudioModels.find(candidate => candidate.key === model || candidate.display_name === model);
          if (!lmStudioModel) {
            onDone(`Model '${model}' not found in LM Studio`, {
              display: 'system'
            });
            return;
          }
          const switchResult = await prepareLmStudioSwitch(lmStudioModel.key);
          setModel(lmStudioModel.key, switchResult);
        } catch (error) {
          onDone(`Failed to switch LM Studio model: ${(error as Error).message}`, {
            display: 'system'
          });
    ...
    function setModel(modelValue: string | null, lmStudioSwitchResult: SwitchLmStudioModelResult | null = null): void {
      setAppState(prev => ({
        ...prev,
