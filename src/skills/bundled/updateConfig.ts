...
Hooks run commands at specific points in Shlomo Code's lifecycle.
...
| Stop | - | Run when Shlomo stops (including clear, resume, compact) |
...
Modify Shlomo Code configuration by updating settings.json files.
...
User: "Format my code after Shlomo writes it"
...
6. **Use --debug** - Run \`shlomo --debug\` to see hook execution logs
`
...
export function registerUpdateConfigSkill(): void {
  registerBundledSkill({
    name: 'update-config',
    description:
      'Use this skill to configure the Shlomo Code harness via settings.json. Automated behaviors ("from now on when X", "each time X", "whenever X", "before/after X") require hooks configured in settings.json - the harness executes these, not Shlomo itself, so memory/preferences cannot fulfill them. Also use for: permissions ("allow X", "add permission", "move permission to"), env vars ("set X=Y"), hook troubleshooting, or any changes to settings.json/settings.local.json files. Examples: "allow npm commands", "add bq permission to global settings", "move permission to user settings", "set DEBUG=true", "when shlomo stops show X". For simple settings like theme/model, use Config tool.',
    allowedTools: ['Read'],
    userInvocable: true,
