...
import { LOCAL_INSTRUCTIONS_FILE, PROJECT_INSTRUCTIONS_FILE } from '../../utils/instructionsFiles.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getClaudeConfigHomeDir(), PROJECT_INSTRUCTIONS_FILE);
  const projectMemoryPath = join(getOriginalCwd(), PROJECT_INSTRUCTIONS_FILE);
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = `Saved in ~/.shlomo/${PROJECT_INSTRUCTIONS_FILE}`;
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./${PROJECT_INSTRUCTIONS_FILE}`;
      } else {
        if (file.parent) {
