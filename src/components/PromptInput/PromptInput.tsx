...
import { switchLmStudioModel } from '../../services/lmStudio/modelManagement.js';
type Props = {
  debug: boolean;
...
function PromptInput({
  ...
  const companionCols = companionReservedColumns(columns, companionSpeaking);
  const textInputColumns = columns - 3 - companionCols;
  const bannerColumns = columns - companionCols;
  ...
  const handleModelSelect = useCallback(async (model: string | null, _effort: EffortLevel | undefined) => {
    let lmStudioSwitchResult: Awaited<ReturnType<typeof switchLmStudioModel>> | null = null;
    if (model && isLmStudioBackend()) {
      try {
        lmStudioSwitchResult = await switchLmStudioModel(model);
      } catch (error) {
        addNotification({
          key: 'model-switch-failed',
          jsx: <Text color="error">Failed to switch LM Studio model: {errorMessage(error)}</Text>,
          priority: 'immediate',
          timeoutMs: 5000
        });
        return;
    ...
    if (lmStudioSwitchResult?.unloadedInstanceIds.length) {
      message += ` · Unloaded ${lmStudioSwitchResult.unloadedInstanceIds.length} ${lmStudioSwitchResult.unloadedInstanceIds.length === 1 ? 'other model' : 'other models'}`;
    }
    if (lmStudioSwitchResult && !lmStudioSwitchResult.alreadyLoaded) {
