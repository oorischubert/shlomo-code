...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${SHLOMO_TUI_VERSION} (Shlomo Code)`);
    return;
  }
  ...
  if (args[0] === 'unload') {
    profileCheckpoint('cli_unload_path');
    const {
      enableConfigs
    } = await import('../utils/config.js');
    enableConfigs();
    ...
    } = await import('../services/lmStudio/modelManagement.js');
    try {
      const unloadedInstanceIds = await unloadAllLmStudioModels();
      // biome-ignore lint/suspicious/noConsole:: intentional CLI output
      console.log(unloadedInstanceIds.length === 0 ? 'No LM Studio models are currently loaded.' : `Unloaded ${unloadedInstanceIds.length} LM Studio ${unloadedInstanceIds.length === 1 ? 'model' : 'models'}.`);
      return;
    } catch (error) {
      // biome-ignore lint/suspicious/noConsole:: intentional CLI output
      console.error(`Failed to unload LM Studio models: ${error instanceof Error ? error.message : String(error)}`);
      process.exitCode = 1;
      return;
    }
  }
  ...
  if (args[0] === 'resume') {
    process.argv = [...process.argv.slice(0, 2), '--resume', ...args.slice(1)];
